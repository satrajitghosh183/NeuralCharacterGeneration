...
**VULKAN GAME + ENGINE SKINNING (2026-06-26):** `apps/ncg_game` — a native **Vulkan** game ("Rock Walker": loads the rigged `.glb`, LBS-skinned animation, grid floor, orbit camera, WASD + gravity/jump; vk-bootstrap/VMA/tinygltf/GLFW/glm via FetchContent). Builds on a display machine (M4 Pro/MoltenVK or Linux+GPU), NOT the headless H100 — `apps/ncg_game/README.md`. `unity/` — GS-skinning component (`GaussianSplatSkin.compute` + `GaussianSplatRenderer.cs` + `SplatQuad.shader`) deforms the splats with the same SMPL-X skeleton as the mesh; **proven correct** by `tests/fit/test_gs_skinning.cpp` (top-4 bone-weight skinning == `deform_avatar`, <1e-5, green). Full deliverable report: `docs/REPORT.md`. Memory note: the autograd soft renderer scales with N·H·W → `--densify` at high res can OOM (hit + reverted; bound densify or use higher res without densify).

**PLAYABLE ENGINE CHARACTER GREEN on H100** (2026-06-26): `ncg_cli avatar` now exports a game-ready character in **two co-registered representations sharing one 55-joint SMPL-X skeleton** (`docs/engine_character.md`): (1) `<prefix>.ply` — standard 3DGS binary splat (Inria convention: f_dc/opacity/scale/rot) via `mesh::write_gaussian_ply`, the high-fidelity render asset for any GS plugin/viewer; (2) `<prefix>.ply.skin` — per-splat top-4 bone idx+weights (from each splat's bound vertex's SMPL-X `lbs_weights`) so splats skin to the rig; (3) `<prefix>.glb` — rigged + animated SMPL-X mesh, the universal physics-ready body (ragdoll/colliders in any engine). Both reps deform from the same skeleton → physics drives both. `fit_avatar` gained optional **adaptive densification** with per-Gaussian vertex binding (`--densify 1`), and returns `AvatarFitResult{cloud, binding}`. Validated end-to-end on The Rock: `ncg_cli avatar --frames data/rock_all --motion data/rock_styled.npy --out-prefix rock_char` → rock_char.{glb,ply,ply.skin}, **20.8 dB** body-masked, all three structurally validated (glb: 55 joints/65-frame anim; ply: 10475 finite splats; skin: weights sum→1). **Remaining for in-engine GS: a Unity/Unreal GS-skinning component** (compute shader reading the .skin against skeleton bone matrices — the `deform_avatar` math); the mesh+physics path works with zero custom code.

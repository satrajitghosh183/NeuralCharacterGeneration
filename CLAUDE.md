...
**NeRF leg GREEN on H100** (2026-06-21, 4/4 nerf tests pass; total 33/36 with 2 by-design skips): `ncg-nerf` (`TinyNerf` MLP + differentiable `render_volume` + premultiplied `composite_over` hybrid + `fit_nerf_to_views`), `tests/nerf/test_nerf.cpp`, `ncg_cli nerf`. 3DGS + NeRF + hybrid now coexist. Also fixed a `ctest` LABELS bug (multi-label tests dropped every label after the first). **Known follow-up:** single-view `fit_nerf_to_views` plateaus at the background solution (dead-ReLU collapse) — needs optimizer hardening before the NeRF render is presentable; tests + pipeline are correct.
